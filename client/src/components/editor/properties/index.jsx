...
import { cloneSelectedObject, deletedSelectedObject, replaceShapeWithImage } from '@/fabric/fabric-utils';
import { fetchWithAuth } from '@/services/base-service';
import { useEditorStore } from '@/store';
import {  AlignHorizontalJustifyCenter, AlignHorizontalJustifyEnd, AlignHorizontalJustifyStart, AlignLeft, AlignRight, AlignVerticalJustifyCenter, AlignVerticalJustifyEnd, AlignVerticalJustifyStart, Bold, Copy, FlipHorizontal, FlipVertical, Italic, Lock, MoveDown, MoveUp, Trash, Underline, Unlock } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const handleSelectionCreated = () => {
      ...
      if (activeObject) {
        ...
        setisLocked(activeObject?.locked)
        setLinkUrl(activeObject?.link)
...
// fetch once when shape selected
useEffect(() => {
  if (objectType === "shape") {
    fetchUserUploads();
  }
}, [objectType, fetchUserUploads]);
...
const handleToggleLock = () => {
  ...
  const newLockState = !selectedObject.locked;
setisLocked(!selectedObject.locked)
  ...
  selectedObject.set({
    ...
    lockUniScaling: newLockState,
    locked: newLockState,
  });

  canvas.renderAll(); // better than renderAll()
  // markAsModified();
};
...
const handleAlignment = (position) => {
  ...
  selectedObject.setCoords(); // ✅ important
  canvas.renderAll();
};
...
const handleLinkChange = (e) => {
  ...
  selectedObject.set("link", url || null); // custom property
  canvas.renderAll();
  markAsModified();
};
...
{/* Lock Element */}
        <div className="flex flex-col gap-2  ">
          <h3 className="text-sm font-medium">Lock</h3>
  <Button
...
<div className="space-y-4 pt-4 border-t">
  ...
  >
    <AlignVerticalJustifyStart className="w-4 h-4" /> Top
  </Button>
  ...
  >
    <AlignVerticalJustifyCenter className="w-4 h-4" /> Middle
  </Button>
  ...
  >
    <AlignVerticalJustifyEnd className="w-4 h-4" /> End
  </Button>
  ...
  >
    <AlignHorizontalJustifyStart className="w-4 h-4" /> Left
  </Button>
  ...
  >
    <AlignHorizontalJustifyCenter className="w-4 h-4" /> Center
  </Button>
  ...
  >
    <AlignHorizontalJustifyEnd className="w-4 h-4" /> Right
  </Button>
  </div>
</div>
  ...
  />
        ...
        {objectType === "shape" && (
         <div className="space-y-4 p-4 border-t">
    <h3 className="text-sm font-medium">Apply Image</h3>
  {isLoading ? (
    ...
    <div className="grid grid-cols-3 gap-2">
      {userUploads.map((img) => (
        <div
          key={img._id}
          className="w-16 h-16 border rounded-md overflow-hidden cursor-pointer hover:ring-2 hover:ring-blue-500"
          onClick={() => handleSetShapeImage(img.url)}
        >
